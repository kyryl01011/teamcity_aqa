...
class TestAuthorizeAgent:
    ...
    def test_authorize_agent(self, super_admin):
        agents_list = super_admin.api_manager.agents_api.get_unauthorized_agents().json()['agent']
        assert len(agents_list) > 0, f'List of unauthorized agents is empty: {agents_list}'
